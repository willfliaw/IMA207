...
def matlab2imz(fichier, namechamp):
    try:
        ...
        print("Erreur dans le nom du .mat")
        return 0, 0, 0
    try:
        ...
        print("Erreur dans le nom du champ du .mat")
        return 0, 0, 0
...
def imz2matbase(namima, listeparam):
    ...
    try:
        ...
        print(legx)
        print("Echec à l'appel de imz2matbase")
        return 0, 0, 0, 0
    ftest.close()
    ...
    if nparam != 7:
        print(
            "Echec appel imz2matbase : nombre erroné de paramètres (%d" % nparam
            + " au lieu de 7"
        )
        return 0, 0, 0, 0
...
def imz2mat(imgName, ncan=0):
    ...
    print("imz2mat : version janvier 2018.  Fichier à ouvrir : %s" % imgName)
    ...
    try:
        ...
        print(legx)
        print("Echec à l'appel de imz2mat")
        return 0, 0, 0, 0
    ...
    if nplantotZ > 0:
        print(
            "Dans ximaread : lecture du canal " + "%d" % ncan + "/" + "%d" % nplantotZ
        )
...
def _imaread(imgName, extension, ncan):
    ...
    if extension == ".rvb":
        ...
        nbBytes = 1
    if nktemps > 0:
        offset, nbBytes, type, komplex, radar, indienZ, namerien = _readDimparamZV2(
            imgName + ".dim"
        )
        if indienZ != "Z":
            indien = indienZ
    nblok = (
        nblok * nbBytes * (1 + komplex)
    )  ######## ATTENTION : ne marchera que pour des tivoli
    return _readImage(
        imgName + extension,
        w,
        h,
...
def _cxbread(imgName, extension, ncan):
    ...
    if w == 0:
        ...
        return 0, 0, 0, 0, 0
    print(
        "image en complex signed char", w, " ", h, "  canaux:", nk, " verif : ", nktemps
    )
    nbBytes = 1
    offset = 0
    nblok = w * h * 2
    ...
    if extension == ".cxbadts":
        ...
        komplex = 3
    if nktemps > 0:
        offset, nbBytes, type, komplex, radar, indien, namerien = _readDimparamZV2(
            imgName + ".dim"
        )
    if radar == 1:
        komplex = 11
...
def _readImage(
    imgName, w, h, nkparam, nktemps, offset, typeA, nbBytes, komplex, nblok, ncan
):
    print(
        "lecture de ",
        imgName,
    ...
    if nkparam < 0:
        print("Fichier RNSat : procédure en test")
        nk = -nkparam
        tagRNSAT = 1
    ...
    try:
        ...
        if nk > 1:
            print(
                "Boucle de lecture entre %d" % nkmin
                + " et %d" % nkmax
                + "   sur %d" % nk
                + " canaux"
            )
        ...
        for nkz in range(nkmin, nkmax):
            ...
            #############
            if komplex == 0:
                print("Données réelles. Nouvelle version de imz2mat  " + "%s" % typeA)
            ...
            if komplex == 3:  # d'abord l'amplitude, puis la phase
                ...
                for i in range(0, h):
                    for j in range(0, w):
                        img[i, j] = imgampli[i, j] * (
                            cos(imgphase[i, j]) + sin(imgphase[i, j]) * 1j
                        )
            ###################################################################################
        ...
        if tagRNSAT == 1:  # je ne m'en suis pas sorti avec les reshape
            ...
            for iut in range(nk):
                isk = iut * iblocRNSAT
                for jut in range(h):
                    imgtot[jut, :, iut] = imgtotstep[
                        jut * w + isk : (jut + 1) * w + isk
                    ]
...
def dat2mat(imgName):
    ...
    try:
        ...
        legx = imgName + ": est un fichier non ouvrable"
        print(legx)
        print("Echec à l'appel de dat2mat")
        return 0, 0, 0, 0
...
def dimdat(imgName):
    ...
    try:
        ...
        legx = imgName + ": est un fichier non ouvrable"
        print(legx)
        print("Echec à l'appel de dimdat")
        return 0, 0, 0, 0
...
def cos2mat(imgName):
    ...
    try:
        ...
        legx = imgName + ": est un fichier non ouvrable"
        print(legx)
        print("Echec à l'appel de cos2mat")
        return 0, 0, 0, 0
    ...
    print("Image Terrasar-X  format DLR.  ncol=%d  nlig=%d" % (ncol, nlig))
...
def urlimz2mat(nomurl, ncanselect):
    ...
    if nomurl.startswith("http") == True:
        ...
        print("%s >> Ce nom n" "est pas celui d" "une url" % nomurl)
        return 0, 0, 0, 0, 0
    ...
    if versionpython[0] == "2":
        try:
            ...
            print("Erreur à la lecture de %s :" % namedim, e.reason)
            return 0, 0, 0, 0, 0
    ...
    if nn > 2:  # le .dim a une première ligne "complète"
        nplan = int(datadim[2])
        print("Multitemporal image %d data" % nplan)
        if nomurl.endswith(".ima"):
            print("Fichiers Unix")
            endian = ">"
        if nomurl.endswith(".IMA"):
    ...
    print(
        "lecture .dim OK -> largeur:%d hauteur:%d profondeur:%d"
        % (largeur, hauteur, nkan)
    )
    ...
    if tagcmplx == 0:
        ...
        print("Debug %d %d %d" % (tagcmplx, nbBytes, largeur))
    ...
    if tagcmplx == 2:
        ...
        if nkan > 1:
            if ncanselect == 0:
                img = npy.zeros([hauteur, largeur, nkan]) + 1j * npy.zeros(
                    [hauteur, largeur, nkan]
                )
            else:
                img = npy.zeros([hauteur, largeur]) + 1j * npy.zeros([hauteur, largeur])
    ...
    if versionpython[0] == "2":
        try:
            ...
            print("Erreur à la lecture de %s :" % nomurl, e.reason)
            return 0, 0, 0, 0, 0
        dataficima = urllib.urlopen(nomurl)
    ...
    if nkan == 1:
        ...
        if tagcmplx == 2:
            for i in range(0, hauteur):
                ...
                img[i, :] = (
                    imgligne[0 : 2 * largeur : 2] + 1j * imgligne[1 : 2 * largeur : 2]
                )
        ...
        if tagcmplx == 2:
            for nk in range(nkan):
                for i in range(0, hauteur):
                    ...
                    imgligne = npy.ndarray(2 * largeur, endian + typeA, record)
                    img[i, :, nk] = (
                        imgligne[0 : 2 * largeur : 2]
                        + 1j * imgligne[1 : 2 * largeur : 2]
                    )

    if nkan > 1 and ncanselect > 0:
        print("Sélection du canal %d/%d" % (ncanselect, nkan))
        if tagcmplx == 1:
...
def visusarbase(tabimaparam, paramseuil, tagspe, *therest):
    ...
    if RSI < 16:
        legspe = (
            "Pas de visualisation : Tableau manifestement beaucoup trop petit (%d) pour etre une image : pas d"
            "affichage" % (RSI)
        )
        print(legspe)
        return 0
    ...
    if ZZ == 3:  # on traite à part les couleurs
        ...
        if tagspe > 0:
            plt.figure()
            if ipart == 1:
                print("Figure réduite pour l'image")
        plt.imshow(tabima / 255.0, interpolation="nearest")
        if tagspe > 0:
            plt.title(malegende)
            plt.show()
        return 0
    ...
    valsig = npy.std(BB)
    valmoy = npy.mean(BB)
    legx = "Min %.3f   Max %.3f    Moy %.3f   Ect %.3f " % (
        valmin,
        valmax,
        valmoy,
        valsig,
    ...
    if type(paramseuil) == list:
        ...
        if kparamhisto == 0:
            kparamhisto = kparam
        print(
            "seuil min (%.1f) et seuilmax (%.1f) passés en argument. kparam = %.1f"
            % (seuilmin, seuilmax, kparam)
        )
        malegende = "Image affichée entre %.1f et %.1f  (vmoy + %.1f vsig)" % (
            seuilmin,
            seuilmax,
            kparam,
        )
        ...
        if kparam > 0:
            ...
            malegende = (
                "Image  [%.2f, %.2f] seuil %.2f \n   valmoy (%.3f) + %.3f sigma (%.3f) "
                % (seuilmin, valmax, seuilmax, valmoy, kparam, valsig)
            )
    ...
    if tagspe > -1:
        ...
        if ipart > 0:
            if tagspe == 0:
                print("Affichage pas conforme (appel de l" "histogramme en mode Z)")
        ...
        if ipart > 0:
            ...
            fntot = float(ntot)
            fnoutlierpourcent = float(noutlier) * 100.0 / fntot
            ...
            if npy.isrealobj(tabima) == True:
                print(
                    "Histogramme des valeurs d"
                    "une image (%.3f %.3f) entre %.3f et %.3f"
                    % (npy.min(tabima), npy.max(tabima), seuilmin, seuilmaxhisto)
                )
                ...
                resulvoid = plt.hist(
                    tabima.ravel(),
                    range=[seuilmin, seuilmaxhisto],
                    bins=256,
                    normed=True,
                    color="wheat",
                )
            else:
                print("Histogramme des valeurs absolues d" "une image complexe")
                #                plt.hist(npy.reshape(npy.abs(tabima),ntot), range=[seuilmin,seuilmaxhisto], bins=256, normed=True, color='wheat')
                resulvoid = plt.hist(
                    npy.abs(tabima.ravel()),
                    range=[seuilmin, seuilmaxhisto],
                    bins=256,
                    normed=True,
                    color="wheat",
            ...
            if kparamhisto > kparam and seuilmax < valmax:
                ...
                fnoutlierhistopourcent = float(noutlierhisto) * 100.0 / fntot
                ###                plt.xlim([0.,seuilmaxhisto])
                baratinx = (
                    "%d pixels > %.1f (%.1f  %%) ,   %d pixels > %.1f (%.3f  %%)"
                    % (
                        noutlier,
                        seuilmax,
                        fnoutlierpourcent,
                        noutlierhisto,
                        seuilmax,
                        fnoutlierhistopourcent,
                    )
                )
            else:
                baratinx = "[%d]    %d pixels > %.1f (%.1f  %%)" % (
                    int(fntot),
                    noutlier,
                    seuilmax,
                    fnoutlierpourcent,
            ...
            if seuilmin > 0.001:
                ...
                if noutliermin > 0:
                    fnoutlierminpourcent = float(noutliermin) * 100.0 / fntot
                    baratinx = (
                        "%d pixels < %.1f (%.1f  %%)      "
                        % (noutliermin, seuilmin, fnoutlierminpourcent)
                        + baratinx
                    )
...
def visuflicker(ima1, ima2):
    ...
    while istop == 0:
        ...
        plt.gcf().canvas.set_window_title(
            "Image %d/%d \n Souris pour défiler, clavier pour arreter"
            % (kut + 1, nkan0)
        )
        #    plt.draw()
        figpile.canvas.draw()
...
def synthese_azimuth(compressed_data, imagename):
    ...
    for t_idx in range(0, len(vec_range)):
        ...
        compensated_data[t_idx, :] = compressed_data_f[t_idx, :] * npy.exp(
            1j * 2 * npy.pi * Lsynt / (2 * vplat) * vect_fd
        )
    compensated_data_t = npy.fft.ifftshift(
        npy.fft.ifft(compensated_data, axis=1), axes=0
    )
