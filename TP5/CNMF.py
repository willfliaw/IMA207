...
def CNMF_init(xdata,ydata,w,M,hyper,multi,delta,I_in,delta_h,delta_m,srf,init_mode=0,mask=0,verbose='off',MEMs=0):
    ...
    if np.isscalar(mask):
        masking = 0
        mask = np.ones((int(hy),int(hx)))
    else:
        masking = 1

    # Initialize H_hyper: (M, N_h)
    if masking == 0:
        H_hyper = np.ones((M, int(hx*hy)))/M
    else:
    ...
    for i in range(I_in):
        ...
        if i == 0:
            ...
            for q in range(I_in*3):
                # Update H_hyper
                H_hyper_old = H_hyper
                H_hyper_n = np.dot(W_hyper.transpose(), hyper) #### TO BE FILLED. Numerator of MU update for H_h
                H_hyper_d = np.dot(np.dot(W_hyper.transpose(), W_hyper), H_hyper) #### TO BE FILLED. Denominator of MU update for H_h
                H_hyper = (H_hyper*H_hyper_n)/H_hyper_d #### MU update for H_h
            ...
            # Update W_hyper
            W_hyper_old = W_hyper
            W_hyper_n = np.dot(hyper[0:band, :], (H_hyper.transpose()))#### TO BE FILLED. Numerator of MU update for W
            W_hyper_d = np.dot(np.dot(W_hyper[0:band,:], H_hyper), H_hyper.transpose())#### TO BE FILLED. Denominator of MU update for W
            W_hyper[0:band, :] = (W_hyper[0:band, :]*W_hyper_n)/W_hyper_d#### TO BE FILLED. MU update for W
            # Update H_hyper
            H_hyper_old = H_hyper
            H_hyper_n = np.dot(W_hyper.transpose(), hyper)#### TO BE FILLED. Numerator of MU update for H_h
            H_hyper_d = np.dot(np.dot(W_hyper.transpose(), W_hyper), H_hyper)#### TO BE FILLED. Denominator of MU update for H_h
    ...
    if masking == 0:
        ...
        for i in range(M):
            tmp = zoom_bi(H_hyper[i,:].reshape(int(hx),int(hy)).copy(),w) # TO BE FILLED initialize H_multi by (row-wise) bilinear interpolation of H_hyper
    ...
    for i in range(I_in):
        if i == 0:
            ...
            for q in range(I_in):
                # Update H_multi
                H_multi_old = H_multi
                H_multi_n = np.dot(W_multi.transpose(), multi) #### TO BE FILLED. Numerator of MU update for H_multi
                H_multi_d = np.dot(np.dot(W_multi.transpose(), W_multi), H_multi)#### TO BE FILLED. Denominator of MU update for H_multi
                H_multi = (H_multi*H_multi_n)/H_multi_d#### TO BE FILLED. MU update for H_multi
...
def CNMF_ite(xdata,ydata,w,M,hyper,multi,W_hyper,H_hyper,W_multi,H_multi,I_in,delta_h,delta_m,I_out,i_out,srf,mask=0,verbose='off'):
    ...
    if masking == 0:
        H_hyper = gaussian_down_sample(H_multi.transpose().reshape(xdata,ydata,M),int(w)).reshape(int(hx*hy),M).transpose()
    else:
        mask2 = zoom_nn(mask,w)
        for q in range(M):
...
def zoom_bi(data,w):
    ...
    cols = data.shape[1]
    # index
    r = np.tile(((2*np.r_[0.:rows*int(w)]+1)/(2*int(w))-0.5).reshape(rows*int(w),1),(1,cols*int(w)))
...
def estR(HS,MS,mask=0):
    ...
    Y = gaussian_down_sample(MS,w,mask2).reshape(rows2*cols2,int(bands1))
